...
- **v1 backend:** one, whichever is least work (probably a C emitter or the JVM). x86, arm64, and JVM were the original targets; three backends plus three FFI targets was recognized as three projects.
...
1. **Soil core.** ML inference, effect rows, sum types and records, derived functions with derivation strategies, fixed-width and big integers, `Utf8`, total-order floats, `panic` effect, no `exn`, no mutation, capability-style `io`, one backend, C-ABI FFI to Rust and Python FFI via embedded CPython, both with hand-written bindings, sequenced C ABI → Rust batteries → Python → Python batteries. One corpus shim example per FFI is part of this milestone, as `trellis bind` depends on them. Runtime written in Rust, exposing a C ABI, with a trivial `main` wrapper from the first commit. No refinements yet.
...
3. **`trellis bind <symbol>`.** Symbol metadata fetching for Python and Rust, the binding context bundle, and the binding lowering job. Must exist before the first project.
4. **Minimal IDE.** Electron-served web app over the daemon: Markdown editor with test-block widgets, graph view, lower button with streaming agent output and the question panel, REPL, lock rendering. As thin as possible.
5. **Use it for a real small project.** Specifically, a program the author would otherwise have had an agent write in pure Python: it uses Rust crates through `soil-rs-std`, binds a dozen Python functions through `trellis bind`, and does real work in Soil, so every half of the pitch is exercised. This is the test of whether writing `.tr` files and reading generated Soil is pleasant.
6. **Then** refinements and demotion, trace-to-test fix mode, profiling, build targets, TOML→Nix, raw-API provider, `trellis derive`, headless CI mode.
